...
//el.loader = $('<div class="gvIILoader"/>').appendTo(el.mainImgContainer);
            ...
            //el.titleTextBox = $('<div class="gvIIAltText"/>').appendTo($container);
			...
			$img.each(function(i){
				...
				var $div = $('<div id="gvIIID'+i+'" class="element other nonmetal">')
				.appendTo($holder)
...
/**/
                   //$image.css({width:$w,height:$h,position:'relative'})
                   $image.css({width:$w,height:$h,left:(wContainer-$w)*.5,top:(hContainer-$h)*.5,position:'relative'})
/**/
					$('<div class="gvIIFlash">').appendTo($div).css({"opacity":".00"}).stop().animate({"opacity":".00"},500)
					.mouseover(
						function(){
							var $f = $(this);
							$f.css({"opacity":".19"}).stop().animate({"opacity":".01"},500);
							/*
	...
	$.fn.jqGalViewII.view = function(img,el){
		...
		$img.onload = function(){
			...
			el.loader.fadeOut('fast',function(){el.image.fadeIn();el.titleTextBox.text(img.title).fadeIn();});
		};
		$img.src = url;
